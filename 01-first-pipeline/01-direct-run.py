...
# ---------------------------------------------------------------------------
# 제출 함수
#    - project / region / pipeline_root 는 환경변수로 받는다.
#      (GCP_PROJECT, GCP_REGION, PIPELINE_ROOT)
#    - 나머지 옵션들은 함수 인자로 받아 기본값을 가진다.
# ---------------------------------------------------------------------------
def submit_pipeline(
    template_path: str,
    ...
    # project / region / pipeline_root 는 os.environ 에서 직접 읽는다.
    # 사전에 셸에서 `export GCP_PROJECT=... GCP_REGION=... PIPELINE_ROOT=...` 로 지정.
    ...
    display_name = (
        display_name
        or f"three-component-demo-{datetime.now(UTC):%Y%m%d-%H%M%S}"
    )
    ...
    # 비동기 제출 — 완료 여부는 기다리지 않고, 제출이 수락됐는지만 확인한다.
...
if __name__ == "__main__":
    ...
    # GCP_PROJECT / GCP_REGION / PIPELINE_ROOT 환경변수가 설정되어 있어야 한다.
    submit_pipeline(template_path=str(output_path))
